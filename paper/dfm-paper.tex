...
% v0.4: geodesic option E12 (static 1PN sector) transcribed from the
%       canon (THEORY_SYNTHESIS v1.7 Sec. 6-5, DERIVATIONS Sec. 15,
%       BOX_UNIVERSE Sec. 5.5 agreed conditions; V18-V20 PASS
%       2026-07-19; author precondition "submit after V20" satisfied).
%       New Sec. IV D; negative claim 5 rescoped to the base dynamics
%       E1'-E11 exactly as adjudicated; Table I Mercury row updated;
%       L1 marked partially resolved. Base-dynamics claims unchanged.
% v0.5: sixth external AI review (ChatGPT/Grok/Gemini, 2026-07-20)
%       adjudicated. Paper-code consistency restored against v1.26:
%       E8R advection now carries k_F (ninth-review engine spec),
%       E12 stated in its per-source implemented form with the m>=40
%       threshold and single-dominant-source domain, first-law domain
%       adds k_F=0, T1 reworded (self-excluded frame undefined),
%       retired presets (mach/drag/twin, v1.24) replaced by explicit
%       configurations or gclock, figures regenerated at the current
%       commit (fig2 caption now matches the actual galaxy preset;
%       fig6 gas run extended to 64000 steps), k_F "physical scale"
%       softened to a no-universal-calibration statement, S_c R_c^2
%       noted as J/M_c, epsilon has length dimensions, V2 labeled an
%       algebraic round-trip, uncertainty definitions clarified.
%       Title change and T->T_eff renaming deferred to the author.
...
\usepackage{hyperref}
\hypersetup{
  pdftitle={Determinacy-Field Mechanics: A Machian Toy Universe with
    Tunable Frame Dragging, Emergent Inverse-Square Gravity, and
    Spin-as-Heat Thermodynamics},
  pdfauthor={Tetsuya Imamura},
  pdfsubject={Educational toy model; Machian mechanics; frame dragging}}
...
A clearly labeled geodesic option (E12), obtained by promoting the
same line element that drives the clocks to the matter action,
recovers the full 1PN advance ($\beta = \gamma = 1$; $+42.98''$/century
for Mercury) with no additional free coefficient, machine-checked
against the analytic formula (V18--V20).
...
effect~\cite{LenseThirring1918}), but the effect is a weak
post-Newtonian correction, suppressed by powers of $1/c^2$, and is
minute---milliarcseconds per year for Earth
satellites, confirmed by Gravity Probe~B and LAGEOS
ranging~\cite{Everitt2011,CiufoliniPavlis2004}.

This paper poses a deliberately counterfactual question and constructs
the smallest working model that answers it:
...
Throughout, ``machine-verified'' means checked by this embedded
automated suite against stated gates or independently derived analytic
formulas---regression-style verification of the implementation, not
independent experimental validation.
The field and frame are set instantaneously by the global
configuration: DFM has no causal propagation and no Lorentz
invariance (Sec.~\ref{sec:negative}).
...
observable.
The geodesic option E12 (Sec.~\ref{sec:e12}), derived from the same
potential's line element, extends the correspondence to the static 1PN
sector (Mercury's perihelion advance, $\beta = \gamma = 1$).
...
run from identical initial conditions (machine-gated time-averaged
gain $0.055$, gate $\ge 0.03$; outer-band ratio $1.082$ at 6000
steps).
...
(qualitative preset demonstrations).
...
The sign and the spin-odd $S_cR_c^2$ dependence ($\propto J/M_c$ for
the disk moment of inertia $I = \tfrac12 M R^2$) parallel the GR
Lense--Thirring periapsis precession, but the radial power
...
advance is not reproduced by the base rules E1$'$--E11
(Sec.~\ref{sec:negative}); the geodesic option E12 recovers it
(Sec.~\ref{sec:e12}).
...
overclaiming, we summarize here the negative claims that are stated in
full in Sec.~\ref{sec:negative}:
...
(5)~its base dynamics E1$'$--E11 does not reproduce the static 1PN
perihelion advance of Mercury (the clearly labeled geodesic option E12
does, Sec.~\ref{sec:e12}); and
(6)~its thermodynamic sector is a qualitative analogy, restricted to
a single particle size, not a quantitative thermodynamic theory.
...
and the analogue-gravity program reviewed by Barcel\'o, Liberati, and
...
mapping with its benchmark calibration from standard observational
inputs (including the Mercury failure
of the base dynamics), the structural differences, and the geodesic
option E12 that recovers the static 1PN sector.
...
Particles are softened point sources for gravity and determinacy, but
carry a finite disk radius $R_i = r_R\sqrt{m_i}$ and moment of
inertia $I_i = \tfrac{1}{2}m_iR_i^2$ for contact and spin.
...
At $k_F = 1$ the particle preserves (in the absence of other
impulses) its velocity \emph{relative to the
...
in the picture students see; the flux (Gauss-law) interpretation of
the field is deliberately suspended, while orbits remain conic
sections.
...
medium, with the same response strength $k_F$ as matter; rule E8R).
...
Remove all particles (with $\Dz = 0$) and the frame is undefined (T1);
the preferred frame is a property of the matter configuration, not of
space.
...
$k_F = k_{\rm rep} = 0$ (check V10) or fixed spins with conservative
spin repulsion (check V10b); the E6$'$ transport conserves momentum
but not kinetic energy, and its work term is not booked, so the closed
first-law claim also requires $k_F = 0$.
When spin and repulsion potential change
...
The update rules E1$'$--E11 below are the complete dynamics of the
base model (the clearly labeled geodesic option E12, off by default,
is described in Sec.~\ref{sec:e12}); the implementation executes them
with a semi-implicit Euler integrator with substeps.
...
law Eq.~(\ref{eq:E4}), since
$\nabla\bigl[m(d^2+\varepsilon^2)^{-1/2}\bigr]
 = -m\,d\,(d^2+\varepsilon^2)^{-3/2}\,\hat{d}$; A6 therefore holds at
finite $\varepsilon$
(machine check V9, residual $2.4\times10^{-4}$), and $\varepsilon$ is
a numerical device, not physics.
...
The advection term carries the same dragging strength $k_F$ as matter
(A5), so $k_F = 0$ is a no-dragging control for light as well; at the
headline value $k_F = 1$ the medium is fully advected.
...
does not deflect.
(For performance, the implementation lets only sources with
$m \ge 40$ bend rays and source the 1PN correction of
Sec.~\ref{sec:e12}---a stated numerical approximation, not a theory
rule; all verification configurations use sources above this
threshold.)
...
\subsection{Weak-field mapping and benchmark calibration}
\label{sec:calib}
...
weak-field formulas; the material-orbit 1PN sector of the base
dynamics does not (the geodesic option E12 recovers it,
Sec.~\ref{sec:e12}).
...
fourth is not reproduced by the base dynamics E1$'$--E11 (negative
claim~5) and is recovered by the geodesic option E12
(Sec.~\ref{sec:e12}).
All values are computed from the inputs of Table~\ref{tab:inputs}
(standard reference values); the GPS-based inverse calibration of
$\Kt$ agrees with
$c^2/G$ to $0.107\%$.}
\begin{ruledtabular}
...
\hline
GPS net clock-rate difference (satellite $-$ ground) &
...
base dynamics: static field $0$ (Newtonian reduction), E6$'$ gives
only a Lense--Thirring-sign regression; geodesic option E12:
$+42.98''$/century ($\beta = \gamma = 1$, Sec.~\ref{sec:e12}) &
$+42.98''$/century &
\textbf{not reproduced} by E1$'$--E11; recovered by E12 \\
\end{tabular}
\end{ruledtabular}
\end{table*}
...
The differences, stated as structural features rather than as small
...
The sign and the spin-odd $S_cR_c^2$ dependence ($\propto J/M_c$)
parallel the GR Lense--Thirring periapsis precession (which is also
negative for
...
$a^{-3/2}$ per orbit, because the constant background comes to
dominate the denominator of the frame rule E3 and screens the
source's frame contribution---a
model-specific prediction).
No choice of $k_F$ can reproduce the static 1PN advance for several
planets simultaneously.
Because the DFM dragging kernel carries a different radial dependence
from the Lense--Thirring effect, no universal physical calibration of
$k_F$ exists; matching magnitudes locally under solar-system
conditions requires $k_F \ll 1$ (of order $10^{-8}$--$10^{-6}$,
depending on the matching condition), so the $k_F = 1$ used in
demonstrations is a deliberate amplification by six or more orders of
magnitude.
\end{enumerate}

\subsection{Geodesic option E12: recovering the static 1PN sector}
\label{sec:e12}
...
\label{eq:line}
\end{equation}
and the natural question---answered affirmatively here for the static
sector---is whether the matter dynamics can be derived from it.
Promoting Eq.~(\ref{eq:line}) to the matter action
$S_m = -mc^2\!\int\!\ud\tau$ and expanding to first post-Newtonian
order in the static, $\vect{u} = 0$ domain yields an additional
acceleration, included in the released implementation as the clearly
labeled \emph{geodesic option} E12 (engine switch \texttt{geoPN}, off
by default), applied per source $j$:
\begin{equation}
\vect{a}_{\rm 1PN} = \frac{\lambda_{\rm PN}}{c_0^2}\sum_j
 \left[\Bigl((\alpha-\tfrac12)v^2 - (1+2\alpha)U_j\Bigr)\nabla U_j
 - (1+2\alpha)(\nabla U_j\cdot\vect{v})\,\vect{v}\right],
\label{eq:e12}
\end{equation}
with $U_j = Gm_j/\sqrt{d_j^2+\varepsilon^2}$ (so $\nabla U_j$ is the
E4 acceleration of source $j$).
This per-source form omits the cross terms of the total-potential
expression $U\nabla U$ with $U = \sum_j U_j$; the two coincide when a
single source dominates, which is the verified domain below.
The metric exponent $\alpha = 3/2$ follows from the full E7R metric
and gives the isotropic 1PN equation of motion with
$\beta = \gamma = 1$; $\alpha = 1/2$ retains only the time-rate part
($\gamma = 0$) and serves as a control; $\lambda_{\rm PN} \in \{0,1\}$
is an A/B switch, not a fitted coefficient.

Because Mercury's real expansion parameter
$GM_\odot/(c^2a) \approx 2.7\times10^{-8}$ is not numerically
resolvable in the present single-precision implementation, the
machine checks keep Mercury's
eccentricity $e = 0.2056$ and amplify the parameter to
$GM/(c_0^2a) = 1.56\times10^{-3}$ ($GM = 150$, $a = 60$, $c_0 = 40$),
subtracting the $\lambda_{\rm PN} = 0$ numerical baseline (the same
policy as check V6; configurations in Appendix~\ref{app:inputs}).
Check V18 measures $+1.764^\circ$/orbit against the analytic
$6\pi GM/[c_0^2a(1-e^2)] = 1.762^\circ$/orbit ($0.1\%$ relative
error); the time-rate-only control V19 gives $1/3$ of the full advance
to $2.0\%$; and V20 gates the full-to-control ratio at $3.06$ (theory
$3$).
The same formula at Mercury's real values gives $+42.98''$/century
($\beta = \gamma = 1$; the time-rate-only value would be
$+14.33''$/century)---recovered under the same single calibration
$\Kt = c^2/G$ with no additional free coefficient.

The claim domain of this first stage is deliberately narrow: a static,
single-dominant-source test-particle problem.
Sources are massive bodies only (the implementation applies
Eq.~(\ref{eq:e12}) only for sources with $m \ge 40$, the same
numerical threshold as the ray tracer); velocities are referred to the
static background ($\vect{u} = 0$); the E6$'$ transport is disabled
for matter under \texttt{geoPN} $= 1$ (the action already contains
Newtonian gravity, so double counting is forbidden); and a pinned
source makes the run an explicitly open system (outside T7).
The 1PN acceleration is applied to matter without a paired reaction,
so the exact conservation statements of T7 are not claimed under
\texttt{geoPN} $= 1$; the verification runs V18--V20 use a pinned
source and are open systems by construction.
Throughout the paper, results from E1$'$--E11 are labeled the
\emph{base dynamics} and results with \texttt{geoPN} $= 1$ the
\emph{geodesic option}; every table entry states which is used.
The unification with $\vect{u} \ne 0$ (E6$'$) in one variational
principle remains the leading open problem (L1, partially resolved;
Sec.~\ref{sec:conclusion}).
With \texttt{geoPN} $= 0$ (the default) every other claim, experiment,
and conservation statement in this paper is unaffected.
...
(version 1.26; the physics engine is unchanged since the E12/E8R
revisions of versions 1.20--1.21) on the same engine; each figure is
regenerated from the simulation by the stated preset, verification
routine, or explicitly stated configuration, with
...
channels (A12$'$; radiated energy leaves the dynamical domain and is
tracked only as a ledger entry---it exerts no radiation pressure);
reservoir work enters only when $\Dz>0$ or pinned
particles are present, and both first-law checks are configured with
neither.
V10 (37 particles, $k_F = 0$, $k_{\rm rep}=0$, all four dissipation
channels active) closes this ledger to $|\Delta E_{\rm tot}|/E_{\rm scale} =
...
the measured residual sits at the single-precision rounding floor (the
engine stores state in \texttt{Float32Array} buffers, so rounding
accumulates over the $O(N^2)$ pair sums) and
...
Interactively, the same scan is reproduced by editing $\Dz$ in a
rotating-ring configuration: interior particles begin to circle with
the shell at small $\Dz$ and ignore it at large $\Dz$.
(The dedicated bucket preset of earlier releases was retired in
version 1.24 pending a box-universe successor; the scan itself is
fully specified by the ring configuration stated in
Fig.~\ref{fig:mach}.)
This is Newton's bucket as an experiment: the answer to ``does the
water know it rotates?'' becomes a computable function of one model
parameter.
...
Generated by \texttt{tools/gen-figures.mjs} from the bucket ring
configuration stated in the script (14 pinned ring sources at
$r = 150$, $m = 80$, $\Omega_{\rm shell}=0.012$) with
...
A disk system is run twice from \emph{identical} initial conditions
with $k_F = 1$ and $k_F = 0$ (the built-in Newtonian control,
Sec.~\ref{sec:newton}), in two configurations: the display preset
\texttt{galaxy} (central $m = 600$ plus $n = 380$ disk particles,
$r \le 260$, $\Dz = 1$) and the reduced verification configuration V11
(same central mass, $n = 140$, $r \le 130$, $\Dz = 0.5$;
Appendix~\ref{app:inputs}).
...
raising the outer $v(r)$; with $k_F = 0$ the control curve shows an
approximately Keplerian decline over the stated radial interval.
The machine gate (check V11) measures the outer-band
($r \in [85,130]$) mean tangential velocity difference of the reduced
configuration, time-averaged over steps 1000--2000: gain $0.055$
(gate $\ge 0.03$) with control drift $0.012$; the figure-generation
gate on the display preset measures an outer-band ratio
$v_{\varphi}(k_F{=}1)/v_{\varphi}(k_F{=}0) = 1.082$ at 6000 steps.
Both numbers come from single seeded configurations; a systematic
multi-seed uncertainty study is left as future work.
...
$\beta = -0.24$ versus $-0.47$ for the control; quoted for the
display preset).
...
Generated from the display preset \texttt{galaxy} (central $m{=}600$,
disk $n{=}380$, $r{\le}260$, near-Keplerian initialization
$\times1.05$, $\Dz{=}1$) in the built-in A/B comparison mode
(outer-band ratio $1.082$ at 6000 steps); the separate reduced
verification configuration V11 ($n{=}140$, $r{\le}130$, $\Dz{=}0.5$)
gates the time-averaged outer-band gain at $0.055$ (gate
$\ge 0.03$).}
\end{figure}
...
An additional, separately specified configuration ($a=120$, $e=0.5$,
$S=+0.01$) gives
...
A long-run rosette configuration (stated in
\texttt{tools/gen-figures.mjs}; formerly the \texttt{drag} preset,
retired in version 1.24 in favor of the geodesic preset
\texttt{mercury}) shows the same physics as a rosette traced by the
orbit trail ($-11.3^\circ$/orbit at its parameters,
Fig.~\ref{fig:drag}a).
...
(a)~Orbit trail over $\sim$8 orbits in the rosette configuration
(central $m{=}600$, $S{=}0.09$, test orbit from $(60,0)$ with
$v_y{=}3.87$): the ellipse precesses opposite to the orbital motion
(regression), tracing a rosette.
...
spacing) and the rosette configuration (168\,000 steps).}
\end{figure}
...
The gravitational term alone is displayed interactively by the preset
\texttt{gclock}: three resting clocks at increasing distance from a
central mass tick at $\tau/t = 0.931/0.965/0.979$, matching the
analytic values (the dedicated twin preset of earlier releases was
retired in version 1.24; the twin configuration itself remains the
machine check V16).
...
interactive counterpart for the gravitational term: preset
\texttt{gclock}.}
\end{figure}
...
(i) a hot/cold split gas (preset \texttt{gas}; a controlled
configuration with self-gravity and dragging off) equilibrates its
...
with a mild overshoot before settling)---the imposed spin-dependent
pair force E5$'$ produces a macroscopic outward stress; what is
\emph{not} imposed is any macroscopic pressure law.
...
The spectral thermometer (check V2) provides an algebraic round-trip
check linking A9 and A12$'$: the Wien inversion
$s(\lambda) = \sqrt{b_m/(I\lambda)}$ recovers the spin used to
generate the radiated color within $5\%$ (a consistency check of the
bookkeeping, not an independent measurement).
...
(single particle size): the initial gap of $4.4$ closes to $0.29$
...
default parameters (64\,000 and 16\,000 steps, respectively) via
\texttt{HP.sim}.}
\end{figure}
...
It is a \emph{computational thought experiment}: the study of the
consequences of a
...
\subsection{Negative claim 5: the base dynamics does not reproduce
...
This claim is scoped to the base dynamics E1$'$--E11 and stands
unchanged.
The clearly labeled geodesic option E12 (Sec.~\ref{sec:e12}) recovers
the full advance ($\beta = \gamma = 1$, $+42.98''$/century) at the
same $\Kt = c^2/G$ with no additional free coefficient (checks
V18/V20, with the time-rate-only control V19 at $1/3$); strong-field
and gravitational-wave phenomena remain outside the model in either
mode (L8, L12).
...
{\bf L1} (action principle: partially resolved---the static
weak-field geodesics of the E7R line element are implemented and
machine-verified as the geodesic option E12 (Sec.~\ref{sec:e12}),
but E6$'$ itself is still an update rule, and the $\vect{u}\ne0$
unification remains the leading open problem, see
Sec.~\ref{sec:conclusion});
...
The static first stage of this program is already implemented and
machine-verified as the geodesic option E12 (Sec.~\ref{sec:e12}): the
static weak-field geodesics of the line element Eq.~(\ref{eq:line})
give PPN $\beta = \gamma = 1$ and recover the 1PN perihelion advance.
The remaining program is to derive matter, clocks, and light in full
from the single action $S = -mc^2\!\int\!\ud\tau$: the field
equations, the $\vect{u}\ne0$ geodesics (the E6$'$ unification), and
self-field exclusion remain undefined; if it succeeds, $k_F$ would
become a rotation-only parameter.
We regard this unification as the model's single most important open
problem.
...
The complete model---engine, twenty-two presets, verification suite,
...
repository (\texttt{v1.26} series); the submission tag and a Zenodo
...
model.
The softening $\varepsilon$ carries dimensions of length but is
numerical regularization only; quoted physical comparisons refer to
the $\varepsilon \to 0$ limit.
...
V18--V20 & $G{=}1$, $\Dz{=}0.05$, $k_F{=}0$, $k_{\rm rep}{=}0$, all
dissipation off, $\Kt{=}10^4$, $c_0{=}40$, \texttt{geoPN}{=}1,
$\varepsilon{=}0.5$; pinned source $m{=}150$ at origin, test
$m{=}0.01$ at $(47.664,0)$ with $v_y{=}1.94787$ ($a{=}60$,
$e{=}0.2056$); three runs $(\alpha,\lambda_{\rm PN}) =
(1.5,1)/(0.5,1)/(1.5,0)$, 80\,000 steps each; periapsis detection
with $r$-gate and minimum spacing \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
\hline
T1 & For $N{=}1$, $\Dz{=}0$: at the particle's own position the
(self-inclusive) frame equals $\vect{v}_1$, and the self-excluded
frame of E6$'$/E7R is undefined ($W_{\rm ext}{=}0$)---no reference
against which single-particle motion could be detected & A3, A4 &
constructive statement & (constructive; enters V12
frame handling) \\
...
CNP & V11 (gain 0.055, gate $\ge$0.03); display-preset ratio 1.082 \\
